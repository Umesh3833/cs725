...
Original file is located at
    https://colab.research.google.com/drive/1olImxyZRdL9a6yw4IsiT0Z_2fh0LDp2F
"""
...
np.random.seed(42)
NUM_FEATS = 75 #feature Selection Used
...
class Net(object):
	...
	def __init__(self, num_layers, num_units):
		...
		for i in range(num_layers):
			...
			if i==0:
				# Input layer
				self.weights.append(np.random.uniform(-0.25, 0.25, size = (NUM_FEATS, self.num_units)))
			else:
				# Hidden layer
				self.weights.append(np.random.uniform(-0.25, 0.25, size = (self.num_units, self.num_units)))
		...
		self.weights.append(np.random.uniform(-0.25, 0.25, size =(self.num_units, 1)))
...
def feature_scaling_selection(df):
  '''
...
def perform_feature_selection():
  ...
  df2 = pd.read_csv(DATA_PATH + 'dev.csv')
  df2.replace([np.inf, -np.inf], np.nan, inplace=True)
  df2.fillna(df2.mean(), inplace=True)
  df3 = pd.read_csv(DATA_PATH + 'test.csv')

  #Split into input (X) and output (y) variables
  X_train = feature_scaling_selection(df1.iloc[:, 1:91])
  y_train = df1.iloc[:, :1]
  X_dev = feature_scaling_selection(df2.iloc[:, 1:91])
  y_dev = df2.iloc[:, :1]
  X_test = feature_scaling_selection(df3)
...
def generate_features_csv():
  '''
  This is a helper function required to generate the features csv fiile having necessary 75 features considered
  '''
  df1 = pd.read_csv(DATA_PATH + 'train.csv')
  df1.replace([np.inf, -np.inf], np.nan, inplace=True)
  df1.fillna(df1.mean(), inplace=True)
  ...
  for i in range(len(a)):
    exist_count = x.count(a[i])
    if exist_count > 0:
      a[i] = "TimberAvg" +str(a[i] - 1)
    else:
      a[i] = "TimbreCovariance" +str(a[i] - 13)

  df = pd.DataFrame(list(zip(*[a]))).add_prefix('Col')
  df.to_csv('features.csv', index=False)

def is_strictly_degrading(arr, delta):
  '''
  The function checks if the dev loss in the past epochs is strictly perfroming better or not
  '''
...
def main():
  ...
  #Handle data
  train_input, train_target, dev_input, dev_target, test_input = perform_feature_selection()
  y_dev_np = dev_target.to_numpy()
  y_train_np = train_target.to_numpy()
  X_dev_np = dev_input.to_numpy()
  X_test_np = test_input.to_numpy()
  ...
  train(
    ...
    y_train_np, 
    X_dev_np, 
    y_dev_np, patience, min_delta
  )
  
  #Testing
  test_pred = get_test_data_predictions(net, X_test_np)
